...
- **popout**
  ...
  If this is set to true, a new move is made available - you may say "pop [column]" to remove a piece from the bottom of the column, as long as that piece is yours. This means games can no longer end on a draw with a filled board, since one player can just pop a piece off, but it does allow for a new draw condition: neither player wins if, after a pop, *both* players have four-in-a-row.
